...
class HIBE_CLLWW12:
    ...
    def keyGen(self, I, MSK, PP):    
        ...
        first_key_comp = [MSK['g2']**exp1[i] for i in range(3)]
        ...
        return {"K_0": first_key_comp, "K_I": leveled_key_comp} 
    ...
    def delegate(self, PP, SK, I):
        ...
        new_first_key = [SK['K_0'][0]*PP['g2d10_d'][0]**(neg_s0), SK['K_0'][1]*PP['g2d10_d'][1]**(neg_s0), SK['K_0'][2]*PP['g2d10_d'][2]**(neg_s0)]
        new_level_comps = []
        for i in range(num_levels):
            id_i = group.hash(I[i], ZR)
            ...
            new_level_comps.append(new_comp)
    ...
    def decrypt(self, CT, SK):
        ...
        for i in range(id_len):
            for j in range(4):
                elt_m *= pair(CT["C_I"][i][j], SK["K_I"][i][j])
        M = CT["C"] / elt_m
        return M
...
def main():
    ...
    """  
    SK3_pr = hibe.keyGen(I3, MSK, PP)
    CT = hibe.encrypt(msg,I3,PP)
    M = hibe.decrypt(CT, SK3_pr)
    print(M)
    
    beg = time.time()
    SK = hibe.keyGen(I3,MSK,PP)
    end = time.time() - beg
    print("Got through key generation in {}".format(end))
    """
